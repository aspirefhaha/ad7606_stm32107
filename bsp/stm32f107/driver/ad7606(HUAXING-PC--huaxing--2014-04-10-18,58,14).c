...
void ad7606_init(void)
{
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_8;  //SPI波特率预分频值为2
...
static unsigned short ad_readbyte(void)                                        //SPI读写数据函数
{	
#ifdef USE_HW_SPI	
	u8 retry=0;	
	...
	for(i=0;i<16;i++){
	 GPIO_WriteBit(AD_SCK_PORT, AD_SCK_PIN, Bit_RESET);
	 retdata <<= 1;
	 if(GPIO_ReadInputDataBit(AD_MISO_PORT,AD_MISO_PIN)==0){

	 }
	 else{
		 retdata  |= 1;
	 }
		GPIO_WriteBit(AD_SCK_PORT, AD_SCK_PIN, Bit_SET);
	}
	return retdata;
#endif
}

void ad_read(unsigned short * buf,int len)
{
	int i = 0 ;
